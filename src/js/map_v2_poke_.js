...
MapHelper.setMarkers_poi_guai = function(arr,show,target) {
       var anchorb = new QQMap.Point(10,10),
        sizeb = new QQMap.Size(10, 12),
      ...
      if(target){
         if(show==1){
          for(var i=0;i<arr.length;i++){
             var target_t=arr[i].spawn_category;
             if(target_t==target){
              poi_arr.push(arr[i]);
             }
             if(target==11){
      ...
      for(var i=0;i<poi_arr.length;i++){
        var lng=poi_arr[i].longitude;
        var lat=poi_arr[i].latitude;
        var id=poi_arr[i].id;
        var dis=poi_arr[i].distance.toFixed(2);
        var lel=poi_arr[i].level;
        var label=poi_arr[i].spawn_category;
        var cat=tuli_title[label].name;
        var num=i+1;
        ...
        if(label!=1){
          var icon = new QQMap.MarkerImage(
            markersArray[label],
            sizeb,
            origin,
            anchorb
       ...
       markers_poi_guai.push(marker_poi);
       ...
       QQMap.event.addListener(marker_poi, 'mouseover', function() {
           InfoWin.open(); 
           InfoWin.setContent('<p style="text-align:center;font-weight:700;">刷怪点'+this.i+'</p><div style="text-align:left;white-space:nowrap;">ID:'+this.id+'<br/>距离：'+this.dis+'<br/>等级：'+this.lel+'</div>'+'<br/>类别:'+this.cat+'');
...
MapHelper.setMarkers_poi = function(arr,show) {
      ...
      for(var i=0;i<poi_arr.length;i++){
       ...
       QQMap.event.addListener(marker_poi, 'mouseover', function() {
           InfoWin.open(); 
           InfoWin.setContent('<p style="text-align:center;font-weight:700;">道馆'+this.i+'</p><div style="text-align:left;white-space:nowrap;">标题:'+this.name+'<br/>距离：'+this.dis+'<br/>等级：'+this.lel+'</div>');
           InfoWin.setPosition(new QQMap.LatLng(this.index_lat, this.index_lng));
           setTimeout(function() {
                InfoWin.close();
            }, 30 * 1000);
...
MapHelper.setMarkers_poi_radio = function(arr,show) {
       var anchorb = new QQMap.Point(12,18),
        sizeb = new QQMap.Size(26, 26),
        origin = new QQMap.Point(0,0),
        icon = new QQMap.MarkerImage(
            "images/marker_zhandian.png",
            sizeb,
            origin,
            anchorb
        );
      ...
      for(var i=0;i<poi_arr.length;i++){
       ...
       QQMap.event.addListener(marker_poi, 'mouseover', function() {
           InfoWin.open(); 
           InfoWin.setContent('<p style="text-align:center;font-weight:700;">补给站'+this.i+'</p><div style="text-align:left;white-space:nowrap;">标题:'+this.name+'<br/>距离：'+this.dis+'<br/>等级：'+this.lel+'</div>');
           InfoWin.setPosition(new QQMap.LatLng(this.index_lat, this.index_lng));
           setTimeout(function() {
                InfoWin.close();
            }, 30 * 1000);
  ...
  MapHelper.setMarkers_poi_radio_cz= function(arr,show) {
       var anchorb = new QQMap.Point(12,18),
        sizeb = new QQMap.Size(26, 26),
        origin = new QQMap.Point(0,0),
        icon = new QQMap.MarkerImage(
      ...
      for(var i=0;i<poi_arr.length;i++){
       ...
       QQMap.event.addListener(marker_poi, 'mouseover', function() {
           InfoWin.open(); 
           InfoWin.setContent('<p style="text-align:center;font-weight:700;">常驻补给站'+this.i+'</p><div style="text-align:left;white-space:nowrap;">标题:'+this.name+'<br/>距离：'+this.dis+'<br/>等级：'+this.lel+'</div>');
           InfoWin.setPosition(new QQMap.LatLng(this.index_lat, this.index_lng));
           setTimeout(function() {
                InfoWin.close();
            }, 30 * 1000);
...
MapHelper.setMarkers_poi_radio_hb= function(arr,show) {
       var anchorb = new QQMap.Point(12,18),
        sizeb = new QQMap.Size(26, 26),
        origin = new QQMap.Point(0,0),
        icon = new QQMap.MarkerImage(
      ...
      for(var i=0;i<poi_arr.length;i++){
       ...
       QQMap.event.addListener(marker_poi, 'mouseover', function() {
           InfoWin.open(); 
           InfoWin.setContent('<p style="text-align:center;font-weight:700;">候选补给站'+this.i+'</p><div style="text-align:left;white-space:nowrap;">标题:'+this.name+'<br/>距离：'+this.dis+'<br/>等级：'+this.lel+'</div>');
           InfoWin.setPosition(new QQMap.LatLng(this.index_lat, this.index_lng));
           setTimeout(function() {
                InfoWin.close();
            }, 30 * 1000);
  ...
  MapHelper.set_poi_animation_guai = function(num) {
    for(var i=0;i<markers_poi_guai.length;i++){
       markers_poi_guai[i].setAnimation(null);
       markers_poi_guai[i].setZIndex(9);
    }
    for(var i=0;i<markers_poi_.length;i++){
       markers_poi_[i].setAnimation(null);
       markers_poi_[i].setZIndex(9);
    ...
    markers_poi_guai[num].setAnimation(QQMap.MarkerAnimation.BOUNCE);
    markers_poi_guai[num].setZIndex(9999);
  };
  ...
  MapHelper.cleanMarkers_poi = function() {
   ...
   MapHelper.cleanMarkers_poi_guai = function() {
    markers_poi_guai.forEach(function(marker) {
      marker.setMap(null);
    });
    markers_poi_guai= [];
  };
